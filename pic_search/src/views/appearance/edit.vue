...
import {getSingleAppearance,updateSingleAppearance} from '../../api/index'
export default {
    mounted() {
      this.id = this.$route.query.id
      getSingleAppearance(this.id).then(res => {
        if(res.data.code === 0) {
          this.formData = JSON.parse(JSON.stringify(res.data.data.list))
        }
      })
    },
    data() {
    ...
    methods: {
        handleConfirm() {
            ...
            updateSingleAppearance(obj).then(res => {
                if(res.data.code === 0) {
